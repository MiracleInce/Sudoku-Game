...
void initWithOtherBoard(int arr[9][9], int other[9][9]){// Ýki tane tahta alýyor ve birindeki bilgileri diðerine kopyalýyor
	int i, j;
	for(i=0 ; i<9 ; i++){
		for (j=0 ; j<9 ; j++){
			arr[i][j]= other[i][j];
...
void printBoard(int arr[9][9]){ // Oluþan tahtayý ekrana bastýrýyor
	int i, j;
	for(i=0 ; i<9; i++){
		for (j=0 ;j<9 ; j++){
			if(arr[i][j] == 0){
...
void showBoards(int board1[9][9], int board2[9][9], int board3[9][9]){//Baþlangýçta olan 3 tane tahtayý yan yana bastýrýyor
	int i, j;
	for(i=0 ; i<9; i++){
...
bool isValidNumber(int arr[9][9], int row, int col, int number){//Girilen sayýnýn ayný satýr, sütun, ve 3x3'lük karede geçerli olup olmadýðýný kontrol ediyor.
...
bool isEnded(int arr[9][9]){// Oyunun sonunda boþ hücre var mý yok mu onu kontrol ediyor.
    int i = 0;
    int j = 0;
